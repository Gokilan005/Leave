...
export const AuthProvider = ({ children }) => {
    ...
    useEffect(() => {
        const checkUserLoggedIn = async () => {
            ...
            if (savedToken) {
                try {
                    ...
                    if (data.role !== 'student') {
                        // Not a student, clear student session
                        logout();
                    } else {
                        const userData = {
                            _id: data._id,
            ...
            // Clean up any legacy shared cookie that causes collisions
    ...
    const login = async (email, password) => {
        try {
            ...
            if (data.role !== 'student') {
                return {
                    success: false,
                    message: `Role mismatch: "${data.role.toUpperCase()}" cannot log into Student Portal. Please visit the Admin/Faculty Portal at http://localhost:3001`
                };
            }
